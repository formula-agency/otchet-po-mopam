...
- проведенные встречи по видимым строкам таблицы встреч Google Sheets, только результат `Прошла успешно`, дата определяется по `Дате создания`
...
- `scripts/shared_plan_store.py` - публикация и применение общего плана
...
Для публикации плана через кнопку `Опубликовать план` есть файл `Шаблон плана МОП.xlsx`.
...
После выбора файла dashboard открывает GitHub для подтверждения публикации. GitHub Action сохраняет план в `manual-data/shared-plans.json`, пересобирает Pages, после чего один и тот же план отображается на всех устройствах и во всех сеансах. Уже открытые вкладки проверяют обновление общего плана каждые 30 секунд. Публиковать и удалять планы могут владельцы и участники репозитория.
...
Файл `Шаблон плана МОП.xlsx` содержит лист `Сводная за месяц`. В строке `Месяц` нужно указать дату первого числа месяца в формате `01.06.2026`; при публикации дашборд применит план к этому месяцу. Плановые колонки шаблона соответствуют видимым метрикам отчета: продажи, встречи, брони, ипотеки и эфир.
